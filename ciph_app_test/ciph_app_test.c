...
void on_job_complete_cb_0 (struct Dpdk_cryptodev_data_vector* vec, uint32_t size)
{
    ...
    {
		int delta = vec[j].op._seq - last_rcv_seq;
		if (delta > 1)
			printf ("delta > 1: %d\n", delta);

		last_rcv_seq = vec[j].op._seq;
...
int main(int argc, char* argv[])
{
    ...
    // create session
    res = ciph_agent_send(conn_id, &job_sess, 1);
	if (0 != res)
      	printf("send error 0\n");
    ...
    printf ("flush 0 ... %d\n", g_size);
	int cc = 0;
    while(g_size < num_pck + num_pck_per_batch + 1 )
	{
      	ciph_agent_poll(conn_id, MAX_CONN_CLIENT_BURST);
	}
    ...
    // close session
	res = ciph_agent_send(conn_id, &job_sess_del, 1);
	if (0 != res)
      	printf("send error 3\n");
