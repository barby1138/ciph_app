...
const uint32_t MAX_tBUFF_SIZE = 9000;
const uint32_t BLOCK_LENGTH = 16;

uint8_t plaintext[1228] = {
0x45, 0x00, 0x04, 0xCC, 0x4B, 0x38, 0x40, 0x00, 0x3F, 0x11, 0x77, 0x1A, 0xAC, 0x10, 0x00, 0x14,
...
0x45, 0x00, 0x04, 0xCC, 0x4B, 0x38, 0x40, 0x00, 0x3F, 0x11, 0x77, 0x1A
};
...
uint8_t iv[] = {
	0x00, 0x00, 0x00, 0x13, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x13, 0x04, 0x00, 0x00, 0x00
};

uint8_t cipher_key[] = {
	0x1A, 0x0F, 0x32, 0x78, 0x2B, 0x36, 0x26, 0x84, 0xCA, 0x72, 0xD5, 0xF3, 0x7C, 0x5A, 0xE7, 0x94
};
...
uint8_t test_buff[MAX_tBUFF_SIZE];

uint8_t test_buff_burst[64][MAX_tBUFF_SIZE];
uint8_t iv_buff_burst[64][16];
											...
											uint32_t* dev_vecs_idxs)
{
	...
	const uint32_t BUFF_SIZE = MAX_tBUFF_SIZE;//1600; // aligned to BLOCK_LENGTH
...
/*
		RTE_LOG(ERROR, USER1, "MBUF %d\n", (0 != (m->ol_flags & EXT_ATTACHED_MBUF)) );
		struct rte_mbuf_ext_shared_info *shinfo = rte_pktmbuf_ext_shinfo_init_helper(
		...
		//rte_pktmbuf_adj(m, vecs[j].cipher_buff_list.buffs[0].length);

		m->ol_flags |= EXT_ATTACHED_MBUF;
		RTE_LOG(ERROR, USER1, "MBUF 2 %d\n", (0 != (m->ol_flags & EXT_ATTACHED_MBUF)) );
...
*/
		...
		if (vecs[j].cipher_iv.length)
		{
			...
			/*
			RTE_LOG(DEBUG, USER1, "SET OPS ind %d", j);
			RTE_LOG(DEBUG, USER1, "SET OPS p iv %X", iv_ptr);
			RTE_LOG(DEBUG, USER1, "SET OPS p da %X", sym_op->m_src->buf_addr);

			print_buff_dbg((uint8_t*) sym_op->m_src->buf_addr, 16);
			print_buff_dbg(iv_ptr, 16);
			*/
		}
	}
													...
													)
{
	...
	for (uint32_t i = 0; i < ops_nb; i++) 
	{
		...
		/*
		RTE_LOG(DEBUG, USER1, "RES OPS ind %d", j);
		RTE_LOG(DEBUG, USER1, "RES OPS p iv %X", iv_ptr);
		RTE_LOG(DEBUG, USER1, "RES OPS p da %X", sym_op->m_src->buf_addr);
							...
							uint32_t* dev_vecs_idxs)
{
	...
	while (ops_enqd_total < dev_vecs_size) 
	{
		...
		if (ops_enqd < ops_needed) //burst_size
			ops_enqd_failed++;
...
int32_t Dpdk_cryptodev_client::test_cipher_burst(long cid, uint64_t seq, uint64_t sess_id, Crypto_cipher_operation op_type)
{
	int32_t res;
	enum { BURST_SIZE = 10 }; // max is 64

    Crypto_operation op[BURST_SIZE];
    memset(&op, 0, BURST_SIZE * sizeof(Crypto_operation));

	for (uint32_t i = 0; i < BURST_SIZE; i++)
	{
		op[i].op.seq = seq;
    	// sess
    	op[i].op.sess_id = sess_id;
    	op[i].op.op_type = CRYPTO_OP_TYPE_SESS_CIPHERING;
		op[i].op.cipher_op = op_type;
	
		op[i].op.op_ctx_ptr = NULL;

		uint32_t BUFFER_TOTAL_LEN = MAX_tBUFF_SIZE; //1 + rand() % 300 ; //1 + rand() % 264;
		...
		BUFFER_SEGMENT_NUM = (BUFFER_SEGMENT_NUM > BUFFER_TOTAL_LEN) ? 
												...
												BUFFER_SEGMENT_NUM;
		*/
		op[i].cipher_buff_list.buff_list_length = BUFFER_SEGMENT_NUM;
		...
		*/
		...
		memcpy(test_buff_burst[i], pt, 1228 /*BUFFER_TOTAL_LEN*/);
		op[i].cipher_buff_list.buffs[0].data = test_buff_burst[i];
		op[i].cipher_buff_list.buffs[0].length = BUFFER_TOTAL_LEN;

		//printf ("BUFFER_TOTAL_LEN %d BUFFER_SEGMENT_NUM %d\n", BUFFER_TOTAL_LEN, BUFFER_SEGMENT_NUM);

		memcpy(iv_buff_burst[i], iv, 16);
		
		static uint32_t counter = 0;
		counter++;
    	iv_buff_burst[i][3] = (uint8_t) (15 + (counter % 5));
    	iv_buff_burst[i][11] = (uint8_t) (15 + (counter % 5));

    	op[i].cipher_iv.data = iv_buff_burst[i];
    	op[i].cipher_iv.length = 16;

		// outbuf
		op[i].op.outbuff_ptr = NULL;
		op[i].op.outbuff_len = 0;
	}

    run_jobs(0, &op[0], BURST_SIZE);

	for (uint32_t i = 0; i < BURST_SIZE; i++)
	{
		if (op[i].op.op_status != CRYPTO_OP_STATUS_SUCC)
		{
			TRACE_ERROR ("cipher op %d FAILED", i);
		}
	}
...
int32_t Dpdk_cryptodev_client::test_cipher(long cid, uint64_t seq, uint64_t sess_id, Crypto_cipher_operation op_type)
{
	...
	*/
	...
	memcpy(test_buff, pt, 1228 /*BUFFER_TOTAL_LEN*/);
...
int Dpdk_cryptodev_client::test(Crypto_cipher_algorithm a, Crypto_cipher_operation ot)
{
	...
	uint32_t retries;
  	...
  	uint32_t pck_size = MAX_tBUFF_SIZE;
