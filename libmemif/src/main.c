...
int get_list_elt(memif_list_elt_t **e, memif_list_elt_t *list, uint16_t len,
                 int key)
{
  int i;
  if (key == -1)
...
int memif_per_thread_set_connection_request_timer(memif_per_thread_main_handle_t
                                                      pt_main,
...
int memif_set_rx_mode(memif_conn_handle_t c, memif_rx_mode_t rx_mode,
                      uint16_t qid)
{
...
int memif_control_fd_handler(int fd, uint8_t events)
{
  ...
  else
  {
    ...
    if (e != NULL)
    {
      ...
      {
        num =
            (((memif_connection_t *)e->data_struct)->args.is_master) ? ((memif_connection_t *)e->data_struct)->run_args.num_s2m_rings : ((memif_connection_t *)e->data_struct)->run_args.num_m2s_rings;
        for (i = 0; i < num; i++)
        {
                 ...
                 uint8_t has_buffers)
{
  ...
  r->region_size = (has_buffers == 0) ? r->buffer_offset : r->buffer_offset + conn->run_args.buffer_size * (1 << conn->run_args.log2_ring_size) * (conn->run_args.num_s2m_rings + conn->run_args.num_m2s_rings);
                        ...
                        uint16_t *count_out)
{
  ...
  if (EXPECT_FALSE(c->fd < 0))
    return MEMIF_ERR_DISCONNECTED;
  uint8_t num =
      (c->args.is_master) ? c->run_args.num_m2s_rings : c->run_args.num_s2m_rings;
                       ...
                       uint16_t *count_out, uint16_t size)
{
  ...
  *count_out = 0;
  ...
  while (count && ns)
  {
    ...
    dst_left = (c->args.is_master) ? ring->desc[slot & mask].length : c->run_args.buffer_size;
    src_left = size;
      ...
      if (EXPECT_FALSE(dst_left == 0))
      {
        if (count && ns)
        {
          ...
          *count_out += 1;
          ...
          b0->desc_index = slot;
          dst_left =
              (c->args.is_master) ? ring->desc[slot & mask].length : c->run_args.buffer_size;
          ring->desc[slot & mask].flags = 0;
        }
        else
        {
                       ...
                       uint16_t headroom)
{
  ...
  if (EXPECT_FALSE(c->fd < 0))
    return MEMIF_ERR_DISCONNECTED;
  uint8_t num =
      (c->args.is_master) ? c->run_args.num_s2m_rings : c->run_args.num_m2s_rings;
  if (EXPECT_FALSE(qid >= num))
    return MEMIF_ERR_QID;
  libmemif_main_t *lm = get_libmemif_main(c->args.socket);
